...
const setUpdatePassword = async (req, res) => {
  ...
  try {
    ...
    if (user.password) {
      ...
      if (!currentPassword) {
        return res.status(400).json({ message: 'Debe proporcionar su contraseña actual para cambiarla.' });
      }
...
const unlinkGoogle = async (req, res) => {
  try {
    ...
    if (!user || !user.password) {
        return res.status(400).json({ message: 'Necesitas tener una contraseña establecida antes de desvincular Google para asegurar el acceso futuro.' });
    }
